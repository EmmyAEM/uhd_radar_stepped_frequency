...
def plotChirpVsTime (signal, title, sample_rate, axs=None):
    ...
    if axs is None:
        ...
        # Reusing externally-provided axes (e.g. a combined figure), so
        # title each subplot instead of stealing the shared figure suptitle.
        fig = axs[0].figure
        axs[0].set_title(f"{title} (Real)")
        axs[1].set_title(f"{title} (Imaginary)")
